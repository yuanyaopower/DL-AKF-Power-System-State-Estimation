...
% IEEE 118-bus Sage-Husa adaptive KF comparison baseline.
%
% Inputs:
%   U, theta, P, Qp
...
%   - The helper functions formHe_large, cal_hx_large,
%     timevaryingnoise, and forceSPD should be available on the
%     MATLAB search path.
...
%% Case-1 nominal R0 used by all cases
delta = 1e-3;
...
Ffun = @(x) F;
hfun = @(x, U_now) cal_hx_large( ...
    x, G, B, line_equ, numbranch, numBus, slack, ...
    U_idx, theta_idx, num_U, num_theta, U_now ...
);
Hfun = @(x, U_now) formHe_large( ...
    x, G, B, numBus, numbranch, line_equ, slack, ...
    U_idx, theta_idx, num_U, num_theta, gen_bus, U_now ...
);
...
for k = 2:col
    ...
    w_buf(:, idxb) = w_k.^2;
    d_buf(:, idxb) = diag(P_post - Fk * P_post_prev * Fk.');
